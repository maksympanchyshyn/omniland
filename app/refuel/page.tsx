    ...
    <div className="flex w-full justify-center items-start">
      <div className="flex flex-col w-[520px] mt-20 bg-slate-800 rounded-xl p-6 border border-gray-600">
        ...
        <div className={`flex flex-col ${isLoading ? 'animate-pulse' : ''}`}>
          ...
          >
            {!isLoading && balance < debouncedAmount && 'Insufficient balance'}
            {!isLoading &&
              balance > debouncedAmount &&
              (isChainValid ? 'Refuel' : `Switch network to ${chainFrom.name}`)}
            {isLoading && 'Loading...'}
          </button>
          <p className="flex justify-center items-center w-full mt-6">
            Powered by&nbsp;
            <Link
